@echo off
REM Windows local.properties generator with proper path formatting
REM This script generates local.properties with Windows double backslash format

echo Generating Windows local.properties...
...
REM Convert to Windows format with double backslashes
set SDK_DIR_FORMATTED=%SDK_DIR:\=\\%
...
REM Generate local.properties content with proper Windows formatting
(
...
echo sdk.dir=%SDK_DIR_FORMATTED%
echo.
...
if defined ANDROID_KEYSTORE_PATH (
    set KEYSTORE_PATH=%ANDROID_KEYSTORE_PATH:\=\\%
    echo release.storeFile=%KEYSTORE_PATH%
) else (
...
echo SDK Directory: %SDK_DIR_FORMATTED%
echo.
